...
public:
	...
	QDockWidget *dock_Geom, *dock_Gray,	*dock_Enhance, *dock_Morp, *dock_Adjust, * dock_Text;
	Mat* raw;
...
public slots:
	...
	// Filter
	void frostedglassImage();					// 模糊滤镜
	void sketchImage();							// 素描滤镜
	void embossImage();							// 浮雕滤镜
	void glareImage();							// 强光滤镜
	// Adjust
	void adjustContrastImage(int rate);			// 对比度调整
	void adjustBrightnessImage(int rate);		// 亮度调整
	// 按钮组
...
private:
	...
	Morp *morpClass;							// 形态学类
	Menu *I_menubar;							// 菜单栏
	Adjust *adjustClass;						// 图像调整类
	Filter* filterClass;						// 滤镜类
