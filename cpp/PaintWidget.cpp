...
void PaintWidget::textChange(QString font, QString text, int size)							// 设置画笔颜色
{
	FONT = font;
	TEXT = text;
	SIZE = size;
}
void PaintWidget::setMat(cv::Mat& raw)
{
	src = raw;
}

cv::Mat PaintWidget::getMat()
{
	return src;
}
...
void PaintWidget::paint(QImage& theImage) {
	...
	{
		QFont font1(FONT, SIZE, QFont::Bold, false);
		font1.setCapitalization(QFont::SmallCaps);
		font1.setLetterSpacing(QFont::AbsoluteSpacing, sizeof(text));
		p.setFont(font1);
		p.setPen(penColor);
		p.drawText((int)(x1 + 10), (int)(y1 + 10), TEXT);
	}
